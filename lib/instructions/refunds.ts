...
- A refund of $${AUTO_APPROVE_MAX_USD} or more, or from a customer with ${MAX_MONTHLY_REFUNDS} or more refunds in the last 30 days, needs requestHumanApproval first. Pass both amountUsd and orderId: the thresholds are checked against them, and the resulting approval is bound to that exact order and amount.
- requestHumanApproval returns { approved, comment, approvalReceipt }. When approved is true, call issueRefund and pass that approvalReceipt through unchanged — it is what proves the approval happened, and it cannot be reconstructed. An approval alone moves no money; the refund is only real once issueRefund succeeds. If approved is false, do not call issueRefund at all.
